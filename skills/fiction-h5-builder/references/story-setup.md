...
Load this reference when the user asks to set up a new writing project or initialize an oh-story-claudecode directory structure.
...
- Book root: use the book title directly as the directory name, no prefix
- Short stories: live under a `短篇/` sibling or standalone directory depending on project layout

## Active Book Pointer

```
.active-book   # file at project root, contains relative path e.g. "我的小说"
```
...
| `covers/<书名>/封面/封面_v1.png` | Yes — if present (generated by `/story-cover`) |
...
6. Write `.active-book` pointing to this book's directory.
