...
1. 验证利基：从 Reddit、Google 自动补全、AnswerThePublic、Product Hunt 等渠道挖掘候选，核实搜索需求三项信号（自动补全确认 + 现有结果有参与度 + ≥10 个支撑页面角度），并通过 AdSense 政策安全评估（禁止类目、高风险类目、广告主需求层级、流量质量风险）逐一过滤。
2. 规划关键词聚类和站点结构。
3. 设计页面模型：读取 brief → 写 Design Read → 定义 token system（色板、字体配对、布局概念、Signature 元素）→ 锁定设计锚点与三档拨号（DESIGN_VARIANCE / MOTION_INTENSITY / VISUAL_DENSITY）→ 定义可扩展但不单薄的页面模板。融合了 taste-skill 的 Brief Inference 与反 AI 默认纪律，以及 frontend-design-skill 的规划-批判-构建流程和写作纪律。
4. 构建有用的站点体验。
