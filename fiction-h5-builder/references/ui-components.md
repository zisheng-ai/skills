...
**Chapter navigation — MUST use hard reload, not client-side routing:**

The "Next" button must trigger a full browser page load so AdSense reinitializes and the browser shows its native loading progress bar. Use `<a href>` **with** an explicit `onClick` override — do NOT use `<Link>` from `next/link` or `router.push()`.

```tsx
// ✅ Correct — forces hard navigation
<a
...
// ❌ Wrong — SPA navigation, AdSense does not reinitialize
<Link href={`/book/${bookSlug}/chapter/${nextChapter}`}>Next →</Link>
```

Why `e.preventDefault()` + `window.location.href` instead of just `<a href>`: Next.js App Router intercepts same-origin `<a>` clicks after hydration and performs soft navigation. The explicit `window.location.href` assignment bypasses this interception and guarantees a true browser reload.
